---
title: "Trump vs. Harris: A 2024 US Presidential Elections Modelling Forecast"
...
The U.S. presidential election is a pivotal moment not only shapes domestic political landscape but also far-reaching its consequences on a global scale. Republicans and Democrats held primary elections across the country earlier in 2024 that eventually filtered down to the main candidates: Donald Trump and Kamala Harris. The polarized political climate and shifting public opinion have made predicting outcomes increasingly challenging, where winning key swing states is important to securing victory.

The main estimand is the projected support percentage and each candidate's winning probability in terms of state-level victories and Electoral College votes. This paper utilizes linear regression model incorporates numeric value of the credibility of pollsters, number of respondents, poll's recency to estimate the probability of each candidate winning in individual states. The objective is to translate these probabilities into a forecast the overall winner of the election. Our findings show a close race, with Trump holding a slight edge in both the popular vote and Electoral College due to projected victories in key swing states.

Trump's path to victory hinges on vital wins in states such as Pennsylvania, Georgia, and Wisconsin, where his projected margins are narrow but statistically significant. Meanwhile, Harris secures strong performances in populous Democratic strongholds like California and New York, but these do not offset Trump’s advantage in less populous yet vital battleground states. This result underscores the significant influence of swing states and the Electoral College system in determining the outcome, particularly in closely contested elections. This contributes to a broader understanding of how statistical modeling can inform political strategy and election analysis.
...
The dataset used in this analysis originates from various polling organizations, as listed in @PollsterRating, that conducted surveys to gauge public opinion on the presidential candidates, Donald Trump and Kamala Harris.

The measurement process begins with the identification of the target population—likely voters—who are sampled to reflect the broader electorate. Polling organizations use various methodologies to gather data, such as telephone interviews, online surveys, and in-person polling. Each methodology has it's own implications for accuracy and representativeness.

Polling organizations aggregate responses and apply statistical methods to estimate the percentages of support for each candidate. This aggregation process converts individual voter opinions into a summary statistic—the Percentage—that reflects the overall sentiment within the sampled population. These become rows in the raw dataset, reflecting the percentage of support for different pollsters, candidates, methodologies, etc.

## Data Cleaning

We first select and rename key variables from raw data to focus on relevant information. The key variables of interest in our analysis include PollScore, SampleSize, NumericGrade, PollRecency, State, and Percentage, with PollRecency being a variable we created.

To address instances where the original data referenced Joe Biden instead of Kamala Harris, we assumed the entries for Biden will goes to Harris in consequences of Democratic Party. This approach is supported by "Harris’s truncated presidential campaign relies heavily upon President Biden’s policy framework and she eaddress the campaign’s weaknesses and win over relevant voter demographics [@RealInstitutoElcano]. Thus, substituting Biden’s polling data with Harris’s ensures continuity in demographic appeal without altering established polling trends. Then we filter to include only polls relevant to the two candidates，Trump and Harris, and features like PollRecency were constructed to weigh recent polls higher. The data was cleaned by removing irrelevant or incomplete observations, and new variables were constructed where necessary.
...
In this analysis, several key predictors were identified to evaluate polling data effectively. The first variable, **PollScore**, is a numeric representation of the pollster's credibility, which stands for "Predictive Optimization of Latent Skill Level in Surveys, Considering Overall Record, Empirically." This score helps assess the error and bias associated with a specific pollster, where lower values (including negative numbers) indicate better credibility [@PollsterRating]. @fig-pollscoredistribution shows whether the majority of pollsters tend to have high or low credibility scores. We see a higher concentration of negative scores, which suggests a trend toward more reputable pollsters in the dataset.

```{r}
#| label: fig-pollscoredistribution
#| fig-cap: This figure shows the distribution of poll scores, which reflect the credibility of pollsters. A higher concentration of negative scores suggests that more reputable pollsters dominate the dataset, with credibility generally improving as the score becomes more negative.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
#| fig-cap: This histogram illustrates the distribution of sample sizes across all polls. The majority of the polls have sample sizes under 2,500.
#| echo: false
#| eval: true
#| warning: false
#| message: false

# create plot for distribution of sample sizes by candidate
...
#| fig-cap: This bar chart shows the number of polls conducted across U.S. states. Notably, battleground states have a higher number of polls conducted, reflecting their significance in the election.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
# create plot for # of polls per state
...
#| fig-cap: This histogram shows the distribution of poll recency, indicating that most polls were conducted relatively recently, with fewer polls extending back to the 2020 election cycle.
#| echo: false
#| eval: true
#| warning: false
#| message: false

# create plot for distribution of poll recency
...
#| fig-cap: This histogram displays the distribution of numeric grades assigned to each poll, with higher numeric grades reflecting more reliable and accurate polling sources.
#| echo: false
#| eval: true
#| warning: false
#| message: false

# create plot for distribution of numeric grades
...
#| fig-cap: This correlation matrix visualizes the relationships between key numerical predictor variables and the outcome variable, Percentage. Notable correlations include a strong negative relationship between PollScore and NumericGrade, and a weak positive correlation between PollRecency and Percentage.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
We considered both linear regression (LM) and generalized linear models (GLM) to predict support percentages from polling data. We chose linear regression for its simplicity, where the coefficients indicate how each predictor affects the response variable directly. Linear regression is effective when the response variable is continuous and normally distributed, which fits our data well. While GLMs offer more flexibility, they can be more complex and harder to interpret. They can also lead to overfitting if you have many predictors.

Linear regression allows us to capture relationships between our predictor variables, while still providing interpretable coefficients. Given the nature of polling data, this approach is both appropriate and commonly used in election forecasting.
...
The model was implemented using R, with out-of-sample testing and Root Mean Square Error (RMSE) calculations applied to assess predictive accuracy as predicted in @tbl-keyestimates. By splitting the data into training and test sets, we evaluated how well the model performs on unseen data. This approach helps ensure that the model generalizes effectively to new data and avoids overfitting.

For **PollScore**, the Trump model has a coefficient of -0.745, while the Harris model shows a coefficient of 0.781, indicating an inverse relationship for Trump and a positive relationship for Harris with the outcome. The standard errors for these coefficients are 0.266 and 0.350, respectively.

For **PollRecency**, the coefficients are -0.003 for Trump and -0.006 for Harris, with both models showing very small negative relationships. The standard errors are minimal, suggesting these estimates are fairly precise.

The **NumericGrade** variablity has a negative coefficient of -0.124 for Trump and a positive coefficient of 1.458 for Harris. This suggests a different relationship between the numeric grade and the outcome for each model, with Harris showing a stronger positive effect.

Other model summary statistics include a sample size of 2117 for the Trump model and 2255 for the Harris model. The R² values indicate that the Trump model explains 64.5% of the variance, while the Harris model explains 51.5%, suggesting the Trump model provides a better fit. The Adjusted R² values are slightly lower but follow the same trend.

The AIC, BIC, Log-Likelihood, and RMSE values all indicate that the Trump model is preferred over the Harris model in terms of model fit, as the Trump model has lower values for these selected metrics.

The full summary statistics for the Harris and Trump models are displayed in @tbl-modelestimatefull in @sec-appx.

```{r}
#| label: tbl-keyestimates
#| tbl-cap: Summary of key model estimates for Donald Trump and Kamala Harris, including coefficients for predictors like Poll Score, Poll Recency, and Numeric Grade, with standard errors for each estimate. Model performance statistics, such as sample size, R², and adjusted R², are also displayed.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
model_list <- list("Trump Model" = lm_model_trump, "Harris Model" = lm_model_harris)
modelsummary(model_list, 
             output = "kableExtra", 
             statistic = "std.error", 
             coef_map = c("PollScore" = "Poll Score", 
                          "PollRecency" = "Poll Recency", 
...
#| tbl-cap: Predicted average and normalized national popular vote percentages for Donald Trump and Kamala Harris based on model outcomes.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
cleaned_data <- cleaned_data %>%
  mutate(
    PredictedTrumpPercentage = predict(lm_model_trump, newdata = cleaned_data),
...
```{r}
#| label: tbl-electoral
#| tbl-cap:  Projected electoral vote totals for Donald Trump and Kamala Harris, based on state-by-state predictions derived from the model. The table summarizes the total electoral votes each candidate is expected to win.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
electoral_votes <- tibble(
  State = c(
    "Alabama", "Kentucky", "North Dakota", "Alaska", "Louisiana",
...
# join predictions to state and votes
...
The predicted outcomes for states where the margin is 4% or less are summarized in @tbl-statewinners. These states are known as our Battleground states (aka swing states). Donald Trump leads in several key battleground states, including Pennsylvania (45.68% for Trump vs. 44.83% for Harris), Georgia (46.94% vs. 43.49%), and Michigan (44.86% vs. 43.87%), while Kamala Harris secures leads in states such as Wisconsin (45.50% for Harris vs. 45.33% for Trump) and Minnesota (44.73% vs. 41.64%). @fig-predictedscoremap is a visual representation of the predicted outcomes for all states and how it would appear as an electoral map.

```{r}
#| label: tbl-statewinners
#| tbl-cap: Summary of predicted electoral outcomes in battleground states where the margin of predicted support between Donald Trump and Kamala Harris is 4% or less, showing the predicted percentage of the vote for each candidate and the winner in each state.
#| echo: false
#| eval: true
#| warning: false
#| message: false

# determine percent breakdown by states where margin is less than or equal to 4
state_winners <- predictions_with_votes %>%
  ...
  mutate(
    ...
    Margin = abs(TrumpPredicted - HarrisPredicted)
  ) %>%
  filter(Margin <= 4)

# display battle ground states margin in table
state_winners %>%
  select(State, TrumpPredicted, HarrisPredicted, Winner) %>%
  kable(
...
#| fig-cap: A map illustrating the predicted outcomes of the 2024 U.S. presidential election by state, based on the model's projections. States are color-coded by the predicted winner, with red representing Donald Trump and blue representing Kamala Harris.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
```{r}
#| label: tbl-sum
#| tbl-cap: Summary statistics of the cleaned polling data, including an average poll score of -0.379, an average sample size of 1,605, and 33.68% average support across 15,829 polls.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
## Predicting 2024 U.S. Election Outcomes Using State-by-State Polling Data
...
## Limitations of the Model: Data Bias, Shifting Voter Demographics, and Excluded Third-Party Candidates
...
For instance, a recent incident involving derogatory comments about Puerto Rico made at a Trump rally has quickly influenced the views of Puerto Rican voters in Pennsylvania. Puerto Rican voters, who make up nearly half a million people in the state, have voiced anger and disapproval, with some even reversing their voting plans [@Hill2024]. WhatsApp groups, morning radio shows, and community discussions have amplified these sentiments, illustrating how quickly backlash can spread through influential networks, particularly in swing states [@Hill2024]. Our model predicts Trump will win Pennsylvania by a margin of 0.8%, which is less than the 3.7% of Pennsylvania's Population that Puerto Ricans make up [@USCensus2020]. @fig-swingstates shows specifically the battleground states that are under contention. Situations like these could easily cause Pennsylvania's 19 electoral votes to swing the other way, turning from red to blue.

```{r}
#| label: fig-swingstates
#| fig-cap: A map illustrating the predicted outcomes of the 2024 U.S. presidential election in battleground states where the margin of predicted support between Donald Trump and Kamala Harris is 4% or less.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
battleground_states <- predictions_with_votes %>%
  select(State, TrumpPredicted, HarrisPredicted) %>%
  mutate(
    Margin = abs(TrumpPredicted - HarrisPredicted),
    Winner = case_when(
...
battleground_states <- battleground_states %>%
  mutate(State = tolower(State))

merged_data <- states_map %>%
  left_join(battleground_states, by = c("region" = "State"))

# map that displays electoral college winners for swing states
map <- ggplot(merged_data, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = Winner), color = "white") +
  scale_fill_manual(values = c("Donald Trump" = "red", "Kamala Harris" = "blue", "NA" = "grey")) +
  labs(
    fill = "Predicted Winner"
  ) +
  theme_void() +
...
## Next Steps: Enhance Prediction Accuracy with Real-Time Data, Demographic Changes, and Third-Party Voting Trend**s**
...
```{r}
#| label: tbl-modelestimatefull
#| tbl-cap: Full model estimates for the Trump and Harris models, including standard errors for each model parameter.
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
The questionnaires used by Times/Siena are designed to capture a broad range of opinions while remaining concise. Interviews are kept under 15 minutes to reduce survey fatigue and maximize response rates [@TimesSienaPoll]. This pollster prioritizes making sure every viewpoint is accurately represented in order to make the questions fair and ensure that respondents are not being swayed in a particular direction. This means the questions must remain objective and clear. However, a survey with limited response options and questions may mean a respondent's view is not captured in its full breadth. It also means that more granular opinions may be overlooked in favor of broader trends.
...
**2024 US Presidential Election Forecast Survey**

**Conducted by:** Sophia Brothers, Deyi Kong, and Rayan Awad Alim\
**Contact:** sophia.brothers\@mail.utoronto.ca

Welcome, and thank you for taking part in the **2024 US Presidential Election Forecast Survey**. Your participation will help us gain insights into voter preferences, influential issues, and factors shaping the election across all 50 states and Washington D.C. 

As a thank you for your time, all survey respondents will be automatically entered into a sweepstakes, with a **chance to win \$500**. One winner will be selected from each state and Washington D.C.

**Your responses will remain strictly confidential** and anonymous, and any data collected will be securely stored and used solely for academic and analytical purposes. No identifying information will be connected to your survey responses.
...
**State of Residence**\
- \[List of all 50 states and Washington D.C.\]
...
1 (Not Certain at All) to 5 (Very Certain)

**How likely are you to vote on election day?**\
...
**Rank the most important issues influencing your vote:**\
- Economy\
- Healthcare\
- Immigration\
- Climate Change\
...
**Section 5: Candidate Perception**

**Rate the following candidates on Trustworthiness:**\
- Kamala Harris: Very low, Low, Neutral, High, Very high\
- Donald Trump: Very low, Low, Neutral, High, Very high

**Rate the following candidates on Competence:**\
- Kamala Harris: Very low, Low, Neutral, High, Very high\
- Donald Trump: Very low, Low, Neutral, High, Very high

**Rate the following candidates on Leadership:**\
- Kamala Harris: Very low, Low, Neutral, High, Very high\
- Donald Trump: Very low, Low, Neutral, High, Very high

------------------------------------------------------------------------

**Section 6: Prior Voting History**
...
Thank you for your time and participation! Your responses will help us understand the key issues shaping the upcoming election. As a reminder, all participants are automatically entered into a sweepstakes for a chance to win \$500, with one winner selected from each state and Washington D.C.
