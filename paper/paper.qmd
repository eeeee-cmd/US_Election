...
required_packages <- c(
  "tidyverse", "sf", "here", "arrow", "ggplot2", "janitor", "purrr", "knitr", "kableExtra", "ggcorrplot"
)
...
ggplot(Select_candidates_cleaned_data, aes(x = PollScore)) +
  ...
  labs(
    title = "Distribution of Poll Scores by Candidate",
    x = "Poll Score",
    y = "Frequency"
  ) +
  theme_minimal() +
...
#| fig-cap: The distribution of how recently a poll was conducted
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
ggplot(cleaned_data, aes(x = PollRecency)) +
  ...
  labs(
    title = "Distribution of Poll Recency",
    x = "PollRecency",
    y = "Frequency"
  ) +
  theme_minimal()
...
ggplot(cleaned_data, aes(x = NumericGrade)) +
  ...
  labs(
    title = "Distribution of Numeric Grade",
    x = "Numeric Grade",
    y = "Frequency"
  ) +
  theme_minimal()
...
#| fig-cap: The correlation between the numerical predictor variables and the percentage of support for a candidate
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
# create table for rmse results per model
rmse_results %>%
  kable(
    caption = "RMSE Results for Candidate Models",
...
state_winners %>%
  ...
  kable(
    caption = "Predicted Percentage of Electoral Votes Per State (Margin ≤ 4%)",
...
map <- ggplot(merged_data, aes(x = long, y = lat, group = group)) +
  ...
  labs(
    title = "2024 US Presidential Election Predictions by State",
...
```{r}
#| label: tbl-sum
#| tbl-cap: Summary statistics for the cleaned poll data
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
#| fig-cap: "Mapped Model-predicted outcomes for 2024 elections by Swing State"
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
map <- ggplot(merged_data, aes(x = long, y = lat, group = group)) +
  ...
  labs(
    title = "2024 US Presidential Election Battleground States",
    fill = "Predicted Winner"
  ) +
  theme_void() +
  theme(legend.position = "right")
